...
@click.option("-k", "--keep-values", default=None)
...
def create(file_uris, file_uris_file=None, prefix=DEFAULTS["prefix"], 
           ...
           s3_config_file=None, identical_dims=None, keep_values=None, 
           compression=None, cache_dir=None):
    """
    ...
    keep_values = keep_values.split(",") if keep_values else None
    ...
    indexer.create(file_uris, prefix, output_path=output_path, identical_dims=identical_dims,
                   keep_values=keep_values, compression=compression, max_bytes=max_bytes)
